// This by me was edited so that HighDPI/Retina displays are supported.
// All the ownership of the code is to the original author.
...
void ImGui_ImplSDLRenderer_RenderDrawData(ImDrawData* draw_data)
{
    ...
    SDL_Window* window = SDL_RenderGetWindow(bd->SDLRenderer);
    SDL_GetWindowSize(window, &width, &height);
    SDL_GL_GetDrawableSize(window, &logicalWidth, &logicalHeight);
    float dpi = (float)logicalWidth / (float)width;

    if (dpi > 1.0f) {
        SDL_RenderSetScale(bd->SDLRenderer, dpi, dpi);
    }
